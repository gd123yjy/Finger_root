...
def get_model():
    ...
    conv_2 = tf.keras.layers.Conv2D(filters=64, kernel_size=2)(dropout_1)
    ...
    dense_5 = tf.keras.layers.Dense(1000, activation=tf.nn.relu, trainable=True)(flatten_5)
    dropout_5 = tf.keras.layers.Dropout(rate=0.5)(dense_5)

    dense_6 = tf.keras.layers.Dense(1000, activation=tf.nn.relu, trainable=True)(dropout_5)

    dense_7 = tf.keras.layers.Dense(6, activation=tf.nn.relu, trainable=True)(dense_6)
